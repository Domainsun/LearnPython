...
class RegisterHandler(RequestHandler):
    ...
    def post(self, *args, **kwargs):  # post 方式处理登陆逻辑
        ...
        if user and password:
            ...
            if res:
                info = sqlalchemyDemo.add_user(user, password)
                if info:
                    dic['code']=0
                    dic['msg']=info
                else:
...
class LoginHandler(RequestHandler):
    ...
    def post(self, *args, **kwargs):  # post 方式处理登陆逻辑
        ...
        if user and password:
            ...
            res = re.search(phone_pat, user)
            if res:
                info = sqlalchemyDemo.query_user(user, password)
                if info:
                    dic['code']=0
                    dic['msg']=info
                else:
...
class SetNormalCookieHandler(RequestHandler):
    '''
        设置cookie，会作为响应头返回给客户端
    '''
    def get(self, *args, **kwargs):
        self.set_cookie("domain11","normalcookie")
        self.write('set cookie successful!')
